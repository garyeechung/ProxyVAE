...
def train_proxyvae(model: Module, train_loader, valid_loader, ckpt_dir: str,
                   x_key: str, beta1: float, beta2: float, device: str,
                   dataset_name: str, tsne_config: dict, epochs: int = 500,
                   lr: float = 5e-3, if_existing_ckpt: str = "resume",
                   comparison_fn=None):
    ...
    wandb.init(project=WANDB_PROJECT, entity=WANDB_ENTITY, group=f"{dataset_name}",
               name=f"proxyvae_beta1_{beta1:.1E}_beta2_{beta2:.1E}", config=config)
    ...
    for epoch in tqdm(range(ckpt_epoch + 1, ckpt_epoch + epochs + 1)):
        ...
        valid_total_loss, valid_recon_loss, valid_kl_loss, comparison, tsne_img = evaluate_model(
            ...
            device=device, comparison_fn=comparison_fn, tsne_config=tsne_config
        )
