...
def train_model(model: VariationalPredictor, train_loader,
    ...
    for batch in train_loader:
        ...
        total_loss, recon_loss, kl_loss = loss_fn(y, y_pred, mu, logvar)
        total_loss.backward()
        optimizer.step()
...
def evaluate_model(model: VariationalPredictor, val_loader,
    ...
    return avg_total_loss, avg_recon_loss, avg_kl_loss, tsne_img
...
def train_vib(model: VariationalPredictor, train_loader, valid_loader,
    ...
    ckpt_path = os.path.join(ckpt_dir, "vib.pth")
    ckpt_best_path = os.path.join(ckpt_dir, "vib_best.pth")
